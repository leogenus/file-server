...
class FileResourceServiceImpl(
  ...
  override fun saveFileResource(clientId: String, fileResourceType: String, multipartFile: MultipartFile): ResponseEntity<FileResourceResult> {
    ...
    val id = dsl.fetchOne("select nextval('${ENTITY_SEQ.name}')").into(Long::class.java)
    val storedName = hashids.encode(id)
  ...
  override fun loadFileResource(code: String, fileName: String): ResponseEntity<Resource> {
    if (StringUtils.isEmpty(fileName))
      throw FileStorageException("Sorry! Invalid fileName=$fileName")

    val downloadName = FilenameUtils.getBaseName(fileName)
    val extension = FilenameUtils.getExtension(fileName)

    if (StringUtils.isEmpty(code))
      throw FileStorageException("Sorry! Invalid code=$code")

    if (StringUtils.isEmpty(downloadName))
      throw FileStorageException("Sorry! Invalid downloadName=$downloadName")

    if (StringUtils.isEmpty(extension))
      throw FileStorageException("Sorry! Invalid extension=$extension")

    return loadFileResource("$code.$extension")
  }
...
private operator fun URL.component1() = port
private operator fun URL.component2() = host

fun getBaseUrl(request: HttpServletRequest): String {
  val requestURL = URL(request.requestURL.toString())
  val (port, host) = requestURL;
